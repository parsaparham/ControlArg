...
$sql="select * from certificates where name=? and id!=$id";
$arr=array($name);
$num=$object->num($sql,$arr);
if($num>0){
    phpAlert("نام گواهینامه تکراری است");
    ...
    die();
                                                  ...
                                                  $cat_name=$res[0]['name'];
                                                  if($name!=$cat_name){
                                                      $path = '../uploads/certificates/' . $cat_name . ".*";
                                                      $filenames = glob($path);
                                                      $file = $filenames[0];
                                                      $fileType = pathinfo($file, PATHINFO_EXTENSION);
                                                      $imageOld = "../uploads/certificates/" . $cat_name . "." . $fileType;
                                                      $imageNew = "../uploads/certificates/" . $name . "." . $fileType;
                                                      rename($imageOld,$imageNew);
                                                  }
                                                  //if Category's name changes Image's name should be changed


                                                  $sql = "update certificates set name='$name', priority='$priority',active='$active' where id=?";
                                                  $arr = array($id);
...
//If Error, show 'Go back' link
    ...
    echo "<a href='certificates.php'>Go back</a>";
}
...
phpAlert("گواهینامه با موفقیت ویرایش شد.");
